...
- Settings → General → Appearance adds Font size: presets Small / Default /
  Large / Extra large plus a percentage slider (80%–150%), persisted as
  optional `AppSettings.fontScale` (`1` = product ramp). The renderer sets
  `--font-scale` so every `--text-*` step scales in proportion.
