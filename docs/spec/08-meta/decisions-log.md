...
| D343 | Custom reading font size | **Settings → General → Appearance gains a Font size row under Font. Presets (Small 12 / Default 14 / Large 16 / Extra large 18) plus a custom integer px field persist as optional `AppSettings.fontSize` (absent = 14, range 12–24). The renderer sets `--reading-font-size` on the root and remaps the `--text-*` ramp inside `.thread-wrap` (transcript) and `.composer-dock` (composer) without a reload. Window Zoom In/Out/Reset stays independent and still scales chrome; sidebar and settings stay on the product ramp. No host protocol or storage schema version bump.** | Chat and composer sizes were a frozen product ramp; window zoom scales chrome together with text. A reading-scoped px preference lets high-DPI and dense-reading users change conversation type without enlarging controls (ADR 0180). |
...
## 2026-09-08 — Custom reading font size (D343)

- Chat transcript and composer sizes were a frozen `--text-*` ramp. Window
  zoom scales chrome together with text and is not a reading preference.
- Settings → General → Appearance adds Font size: presets 12 / 14 / 16 / 18
  plus custom px (12–24), persisted as optional `AppSettings.fontSize`.
  The renderer sets `--reading-font-size` and remaps the ramp inside
  `.thread-wrap` and `.composer-dock` only.
