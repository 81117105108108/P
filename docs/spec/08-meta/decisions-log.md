...
- Decision D349 is recorded as ADR 0186. See `03-runtime/01-ipc-protocol.md`,
  `03-runtime/02-agent-runtime.md`, `04-ux/08-component-spec.md`,
  `04-ux/09-interaction-patterns.md`, and US-UI-60d.

## 2026-09-08 — Source-aware native notification delivery (D350)

- PR #84 added native notifications for asktool, tool-permission, and Plan
  approval prompts by broadening the existing `showNative` handler. That
  broadening also allowed focused background terminal completions to show a
  native banner, which violated E2E-065.
- Decision D350: application-owned terminal calls use `source: "task"` and
  remain unfocused-only. Interactive prompt calls use `source: "interactive"`
  and are suppressed only for the exact prompt session visible in the focused
  window, so a focused different session can be notified. Omitted or unknown
  sources default to `task`; interactive prompts remain outside the durable
  task inbox, and plugin-native notifications stay on their separate API.
- Decision D350 is recorded as ADR 0187. See `03-runtime/01-ipc-protocol.md`,
  `04-ux/08-component-spec.md`, `04-ux/09-interaction-patterns.md`, and
  E2E-065a.
