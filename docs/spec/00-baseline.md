...
> future post-MVP milestone; it does not change the current exclusion. Voice
> capture is panel-local
> and requires an explicit `ui.microphone` grant; it does not expose the MCP
> bearer token to plugin code.
