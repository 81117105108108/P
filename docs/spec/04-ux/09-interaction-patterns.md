...
- Clicking a delegation topology node opens a conversation-like task view in the
  ...
  Task call's selectable description as a right-aligned user message bubble, capped at four lines with an inline
  expand/collapse control for longer tasks, matching the main conversation, and
  its live process on one subtle vertical
  timeline; it does not render separate details, output, or workflow tabs.
