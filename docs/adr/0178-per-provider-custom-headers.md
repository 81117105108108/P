...
- UI is a compact Advanced toolbar (named, custom, and vendor account) with
  common header presets including `User-Agent`, JSON import for either a direct
  header object or `{ "headers": { ... } }`, and case-insensitive merge without
  duplicate rows. At most two header rows are visible; additional rows scroll
  inside the editor. Named display name stacks above the header controls in the
  same single-column Advanced block. First OAuth login does not collect
  headers; they are edited after the account exists.
