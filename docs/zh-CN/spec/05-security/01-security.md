...
- 只绑定 `127.0.0.1`。不存在绑定局域网或公网接口的配置路径，也不会重新打开
...
- 排除密钥 get/set/delete 通道和渲染器专属的原生选择器/对话框通道。操作目录是
  显式的，新增加的 IPC 处理器不会自动暴露。
- 调用委托给现有主进程 IPC 处理器，因此主机可用性、工作区边界、权限检查、输入
  校验和脱敏日志仍是权威边界。通用危险操作和破坏性命名工具要求显式的
  `confirm: true`。
- 请求和序列化结果有大小上限。服务启动失败不会阻止桌面启动，已接受的操作通过
  Electron 常规运行时日志记录。
...
10. 本地 MCP 控制只允许回环、经过 bearer 认证、默认关闭且有界，不能暴露密钥或
    渲染器专属的原生选择器
