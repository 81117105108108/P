...
- **外观**卡：
  ...
  - **字体大小**：档位（小 12 / 默认 14 / 大 16 / 特大 18）加上自定义
    整数 px（12–24）。选择持久化为 `AppSettings.fontSize`；缺失表示 14。
    渲染器设置 `--reading-font-size`，并在会话 transcript（`.thread-wrap`）
    与输入框（`.composer-dock`）内按比例重映射 `--text-*` 阶，无需重载。
    窗口放大/缩小/重置仍独立，会连控件一起缩放。侧栏与设置保持产品字号阶
    （D343 / ADR 0180）
