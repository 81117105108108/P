...
- **前置条件**：干净的 `~/.pi-desktop` 配置，并有一条能看到会话内容和输入框的对话。
- **步骤**：
  1) 打开设置 → 常规，确认外观卡片在字体行下方显示字体大小，默认档位选中，px 字段为 14。
  2) 选择「大」。确认会话正文、标题、代码、工具输出和输入框无需重载即变大，设置卡片、侧栏会话列表和窗口 chrome 保持产品字号。
  3) 在自定义 px 中输入 20 并提交（失焦或回车）。确认阅读区域继续变大，且没有档位保持选中。
  4) 使用放大，再重置缩放。确认窗口缩放仍会放大 chrome，重置后阅读字号仍为 20px。
  5) 重启应用，打开对话，确认 20px 阅读字号仍然生效（`AppSettings.fontSize`）。
  6) 选择「默认」。确认会话与输入框立即回到 14px；重启后默认仍保留。
- **预期**：字体大小为档位 12 / 14 / 16 / 18 加上自定义整数 px 12–24。选择持久化为 `AppSettings.fontSize`（缺失表示 14），并设置 `--reading-font-size`，仅在 `.thread-wrap` 与 `.composer-dock` 内重映射 `--text-*`。窗口放大/缩小/重置保持独立。无效值被拒绝或钳制。无协议/存储版本升级。
