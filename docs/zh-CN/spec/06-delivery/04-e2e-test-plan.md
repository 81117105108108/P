...
- **步骤**：
  1) 打开设置 → 常规，确认外观卡片在字体行下方显示字体大小，默认档位选中，滑杆为 100%。
  2) 选择「大」。确认会话、输入框、设置标签、侧栏会话标题和 Lucide 图标无需重载即按相对阶一起变大，控件显示 115%。
  3) 把滑杆拖到 125%。确认「特大」被选中，全部 `--text-*` 表面和图标继续变大。确认界面没有 px 输入。
  4) 使用放大，再重置缩放。确认窗口缩放仍会放大 chrome，重置后文字比例仍为 125%。
  5) 重启应用，确认 125% 仍然生效（`AppSettings.fontScale` = 1.25）。
  6) 选择「默认」。确认整个界面立即回到 100%；重启后默认仍保留。
- **预期**：字体大小为档位小 / 默认 / 大 / 特大，加上 80%–150%、步进 2.5% 的百分比滑杆。选择持久化为 `AppSettings.fontScale`（缺失表示 1），并设置 `--font-scale` 乘以全部 `--text-*` 令牌和共享 Lucide 图标。窗口放大/缩小/重置保持独立。无效值被拒绝或钳制。无协议/存储版本升级。
