...
#### E2E-213：仅在发版时部署 Vercel 文档站

- **前置条件**：Vercel 项目的 Root Directory 为 `docs`；GitHub Actions 已配置
  `VERCEL_TOKEN`、`VERCEL_ORG_ID` 和 `VERCEL_PROJECT_ID` secrets。
- **步骤**：1）更新一个 PR 并推送普通分支提交，检查 Vercel 结果。2）对匹配的
  `vX.Y.Z` 标签运行 Release 工作流。3）检查 `deploy-docs` 作业和 production
  文档站。
- **预期**：PR 和普通分支的 Git 部署由 `git.deploymentEnabled: false` 关闭，
  不会构建 Preview 或生成 Vercel 状态检查。GitHub Release 发布后，
  `deploy-docs` 使用带 `--prod` 的 Vercel CLI，production 文档站反映该标签内容。
- **关联规格**：`02-architecture/04-documentation-site.md`、
  `06-delivery/06-release-runbook.md`、ADR 0199
- **验收**：品质（发布交付）
- **里程碑**：M6+
- **状态**：源合同已覆盖（`ci-workflow.test.mjs`）；远程 Vercel 部署和 GitHub
  状态行为仍需操作验证（除非明确要求，否则不要在本地运行 E2E）
...
| 验收 | 应用场景 |
|---|---|
| A — 应用程序启动 | E2E-001、E2E-002、E2E-003、E2E-004、E2E-067、E2E-076、E2E-079、E2E-092、E2E-097、E2E-143、E2E-150、E2E-168、E2E-204 |
...
| M6+ | E2E-121、E2E-122、E2E-123、E2E-142、E2E-148、E2E-150、E2E-151、E2E-168、E2E-199、E2E-200、E2E-202、E2E-203、E2E-209、E2E-211、E2E-212、E2E-213 |
...
- **关联规格**：`07-plugins/03-plugin-api.md`、`07-plugins/13-plugin-permissions-matrix.md`、
  `03-runtime/01-ipc-protocol.md`、`03-runtime/06-host-rpc-protocol.md`、ADR 0196、D358
- **验收**：C（会话）、安全、质量
- **里程碑**：M6+
