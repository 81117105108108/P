...
| D343 | 自定义阅读字号 | **设置 → 常规 → 外观在字体行下增加字体大小。档位（小 12 / 默认 14 / 大 16 / 特大 18）加上自定义整数 px，持久化为可选 `AppSettings.fontSize`（缺失 = 14，范围 12–24）。渲染器在根元素设置 `--reading-font-size`，并在 `.thread-wrap`（会话）与 `.composer-dock`（输入框）内按比例重映射 `--text-*` 阶，无需重载。窗口放大/缩小/重置仍独立，会连控件一起缩放；侧栏与设置保持产品字号阶。无协议/存储版本升级。** | 聊天区与输入框字号是产品写死的阶；窗口缩放会把 chrome 一起放大。阅读范围的 px 偏好让高分屏和希望更密/更大阅读的用户只改会话文字，不放大控件（ADR 0180）。 |
...
## 2026-09-08 —— 自定义阅读字号（D343）

- 会话 transcript 与输入框字号是冻结的 `--text-*` 阶。窗口缩放会把 chrome 一起放大，不是阅读偏好。
- 设置 → 常规 → 外观增加字体大小：档位 12 / 14 / 16 / 18 加上自定义 px（12–24），持久化为可选 `AppSettings.fontSize`。渲染器设置 `--reading-font-size`，仅在 `.thread-wrap` 与 `.composer-dock` 内重映射字号阶。
