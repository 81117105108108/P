...
或 `authKind: none` 的提供商）。控件是按提供商分组的下拉选择，不是手打 id：沿用会话
为空值，选项为按提供商显示名分组的 `vendorKey-or-name/modelId`，不再配置中的固定值
仍作为额外选项保留，以免编辑时被悄悄丢掉。在应用外编辑 Markdown 文件时仍允许自定义
模型 ID。多个提供商使用通用或重复的厂商标识时，选项会改用唯一的提供商显示名；如果
显示名也重复，则使用已存储的提供商 ID，确保每个已配置提供商都不会从选择器中消失。
