...
| [ADR 0199：宿主拥有的插件会话导入与归属 API](/adr/0199-plugin-owned-session-api) | 插件历史会话由主机生成 id，并按插件、来源和外部 id 归属 |
| [ADR 0200：显式插件项目 id 与宿主拥有的会话刷新](/adr/0200-plugin-project-ids-and-session-refresh) | 插件可显式绑定主机项目，成功写入由主机通知渲染器刷新 |
